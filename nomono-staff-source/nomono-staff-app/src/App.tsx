...
import { Member, Staff, hasPerm, getLevel } from './lib/types';
  ...
  return(
    <div className="min-h-screen flex flex-col" style={{background:'#FAFAF7'}}>
      <header className="sticky top-0 z-30 flex items-center justify-between px-4 py-3" style={{background:'#003820'}}>
        <div>
          <div className="font-mono text-sm font-bold tracking-[0.2em] text-[#E0DBBC]">NOMONO</div>
          <div className="font-mono text-[8px] tracking-[0.3em] text-[#C39A4B] uppercase">Staff</div>
        </div>
        <div className="flex items-center gap-2">
          <RolePills staff={curStaff} roles={db.roles} small/>
          <button onClick={handleLogout} className="text-[#E0DBBC66] hover:text-[#E0DBBC] ml-1">{IC.logout}</button>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-5 pb-24">
        ...
        {page==='members'&&
          <MembersPage
            members={db.members}
            onView={viewMember}
            onAdd={()=>{setSelId(null);navTo('add-member');}}
            tiers={db.tiers}
          />
        }

        {page==='member-detail'&&selMember&&
          <MemberDetailPage
            member={selMember}
            transactions={db.transactions}
            checkins={db.checkins}
            onBack={goBack}
            onEdit={()=>navTo('member-form')}
            tiers={db.tiers}
          />
        }
        ...
        {page==='settings'&&
          <SettingsPage
            presets={db.presets}
            setPresets={db.setPresets}
            staffList={db.staffList}
            setStaffList={db.setStaffList}
            tiers={db.tiers}
            setTiers={db.setTiers}
            roles={db.roles}
            setRoles={db.setRoles}
            staff={curStaff}
            setStaff={updateStaffProfile}
          />
        }
      </main>
      ...
      <style>{`
        ...
        *{font-family:'DM Mono',monospace;box-sizing:border-box;margin:0}
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{background:'#003820'}}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="font-mono text-3xl font-bold tracking-[0.3em] text-[#E0DBBC] mb-1">NOMONO</div>
          <div className="font-mono text-[10px] tracking-[0.5em] text-[#C39A4B] uppercase">Padel Club · Staff</div>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-[10px] font-mono text-[#C39A4B] uppercase tracking-widest mb-2">Email</label>
            <input
              type="email"
              value={email}
              onChange={e=>{setEmail(e.target.value);setErr('');}}
              className="w-full bg-transparent border border-[#C39A4B44] rounded px-4 py-3 text-[#E0DBBC] font-mono text-sm focus:outline-none focus:border-[#C39A4B] placeholder:text-[#E0DBBC33]"
              placeholder="staff@nomono.id"
            />
          </div>
          <div>
            <label className="block text-[10px] font-mono text-[#C39A4B] uppercase tracking-widest mb-2">Password</label>
            <input
              type="password"
              value={pass}
          ...
          <button
            onClick={go}
            disabled={loading}
            className="w-full py-3 rounded font-mono text-sm font-bold uppercase tracking-widest hover:brightness-110 active:scale-[0.98] disabled:opacity-50"
            style={{background:'#C39A4B',color:'#003820'}}
          >
